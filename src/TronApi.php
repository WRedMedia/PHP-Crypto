...
class TronApi
{
     ...
     * @throws Exception
     */
    function getBlockByLatestNum($limit)
    {
        if (!is_integer($limit) || $limit <= 0) {
            throw new Exception('Invalid limit provided');
        }

        return $this->fullNode->post('wallet/getblockbylatestnum', [
            'num' => $limit
        ])->block;
    }
    ...
    */
    ...
    function createAssetIssue($name, $abbr, $desc, $url, $supply, $trxRatio, $tokenRatio, $start, $end, $limit, $publicLimit, $frozenAmount, $frozenDays, $precision, $from)
    {
        $payload = [
            'name' => bin2hex($name),
            'addr' => bin2hex($abbr),
